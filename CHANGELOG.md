# Changelog

All notable changes to PhotonAct are documented in this file. The format follows
[Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and the project uses
[Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

### Planned

- Add reproducible comparison benchmarks with auditable raw results in v0.3.

## [0.1.1] - 2026-09-21

### Added

- A self-contained local hysteresis explorer generated by `photonact demo`, with input dragging,
  automatic up/down scanning, explicit state and branch display, threshold markers, and optional
  output-power/transmittance switching.
- Clear warnings that generated HTML embeds the plotted data and inherits the curve's data license.
- A synthetic-data explorer preview and task-oriented quick start on the GitHub project page.

### Changed

- Verify HTML generation from both the source checkout and the installed wheel in continuous
  integration.

## [0.1.0] - 2026-09-21

### Added

- Explicit-state `HysteresisActivation` with deterministic lower/upper threshold switching,
  time-major sequence evaluation, batched state, autograd, and serialized threshold buffers.
- Curve metadata for data kind, response quantity, wavelength, polarization, and hysteresis
  thresholds, with finite-value and range validation.
- A deterministic, local-only converter for the legitimately held Figure 4(b) source workbook,
  including provenance hashes and transmittance-to-output-power conversion.
- Hysteresis documentation, example, and tests for switching, reversal, batching, gradients,
  serialization, and workbook parsing.

### Changed

- Reject non-finite curve points and activation inputs.
- Mark bundled `sample_phh` data explicitly as synthetic and add demonstration thresholds.

## [0.0.1] - 2026-09-20

### Added

- CSV and JSON curve loading with metadata and provenance fields.
- Validation for finite values, strict input ordering, branches, and valid ranges.
- Differentiable piecewise-linear `CurveActivation` with explicit extrapolation and normalization.
- A command-line curve inspector, a minimal backward-pass example, and an introductory notebook.
- Synthetic two-branch demonstration data clearly separated from experimental or paper data.
- Tests across Python 3.10-3.12 and an installed-wheel smoke test in CI.
- English and Chinese documentation, contribution guidance, citation metadata, a security policy,
  and a milestone-based roadmap.

[Unreleased]: https://github.com/SoftBread1217/PhotonAct/compare/v0.1.1...HEAD
[0.1.1]: https://github.com/SoftBread1217/PhotonAct/compare/v0.1.0...v0.1.1
[0.1.0]: https://github.com/SoftBread1217/PhotonAct/compare/v0.0.1...v0.1.0
[0.0.1]: https://github.com/SoftBread1217/PhotonAct/releases/tag/v0.0.1
